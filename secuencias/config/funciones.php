<?php
if (isset($_GET['cambiar_estado_reto'])){
$contenidoc = ob_get_clean();
cambiar_estado_reto($_POST['id'],$_POST['estado']);
exit();
}
if (isset($_GET['guardar_elementos_juego'])){
$contenidoc = ob_get_clean();
guardar_elementos_juego(true);
exit();
}
if (isset($_GET['estudiantes_no_matriculados'])){
$contenidoc = ob_get_clean();
estudiantes_no_matriculados($_POST['id_anio'],$_POST['estudiante']);
exit();
}
...
modificar_reto($_POST['cod'],$_POST['dif'],$_POST['nombre']);
exit();
}
...
function formato_hora($fecha){
    $fecha2="";
    if ($fecha!="0000-00-00 00:00:00"){
     $fecha2 =  date("h:i:s a",strtotime($fecha));
    }
    return $fecha2;
}
function lista_estudiantes($id_anio="",$estudiante=""){
 require("conexion.php");
 @session_start();
 if ($id_anio=="") $id_anio=$_SESSION['id_anio_lectivo'];
 $sql4 = "SELECT * FROM `usuarios` INNER JOIN `matricula` ON `usuarios`.`id_usuarios` = `matricula`.`estudiante` and `matricula`.`anio` = '$id_anio' WHERE `usuarios`.`tipo` = 'estudiante'";
$consulta4 = $mysqli->query($sql4);
if ($consulta4->num_rows==0){
echo '<br>Advertencia, no hay estudiantes disponibles para matricular, para crear usuarios haga clic <a href="usuarios.php">Aquí</a>';
}
  ?>
  <select  title="" class="form-control" name="id_estudiante" id="id_estudiante"  onchange="valida_asignacion_secuencia();" required>
<option value="">Seleccione una opci&oacute;n</option>
<?php 

while($row4=$consulta4->fetch_assoc()){ ?>
<option value="<?php echo $row4['id_matricula']?>" <?php 
if ($estudiante == $row4['id_matricula']) echo " selected ";
?>> <?php echo $row4['apellido1']." ".$row4['apellido2']." ".$row4['nombre']?></option>
<?php } ?>
</select>
  <?php
}
function estudiantes_no_matriculados($id_anio="",$estudiante=""){
 require("conexion.php");
 @session_start();
 if ($id_anio=="") $id_anio=$_SESSION['id_anio_lectivo'];
 $sql4 = "SELECT * FROM `usuarios` LEFT JOIN `matricula` ON `usuarios`.`id_usuarios` = `matricula`.`estudiante` and `matricula`.`anio` = '$id_anio' WHERE `usuarios`.`tipo` = 'estudiante' AND `matricula`.`id_matricula` is NULL ";
 if ($estudiante!="")
 $sql4 .= " and `usuarios`.`id_usuarios` != '$estudiante' UNION SELECT * FROM `usuarios` LEFT JOIN `matricula` ON `usuarios`.`id_usuarios` = `matricula`.`estudiante` WHERE `usuarios`.`id_usuarios` = '$estudiante'";
 #echo $sql4;
$consulta4 = $mysqli->query($sql4);
if ($consulta4->num_rows==0){
echo '<br>Advertencia, no hay estudiantes disponibles para matricular, para crear usuarios haga clic <a href="usuarios.php">Aquí</a>';
  ...
  ?>
  <select  title="" class="form-control" name="estudiante" id="estudiante"  onchange="valida_matricula();" required>
<option value="">Seleccione una opci&oacute;n</option>
<?php 

while($row4=$consulta4->fetch_assoc()){ ?>
<option value="<?php echo $row4['id_usuarios']?>" <?php 
if ($estudiante == $row4['id_usuarios']) echo " selected ";
?>> <?php echo $row4['apellido1']." ".$row4['apellido2']." ".$row4['nombre']?></option>
<?php } ?>
</select>
  <?php
  ...
  <?php
    while ($row = $consulta->fetch_assoc()){
         ...
         <tr>
          ...
          <td>
              ...
              $sql_seguimiento_secuencia = "SELECT count(`id_seguimiento_reto`) as numero, reto.id_secuencia, `aprobado` FROM `seguimiento_reto` inner join `reto` on `reto`.`id_reto`= `seguimiento_reto`.`reto` WHERE `seguimiento_reto`.`usuario` = '".$row['id_usuarios']."' group by reto.id_secuencia,seguimiento_reto.`aprobado`";
    $consulta_seguimiento = $mysqli->query($sql_seguimiento);
    $consulta_seguimiento_secuencia = $mysqli->query($sql_seguimiento_secuencia);
    ...
    #$resumen = "<hr>"."Total: ".$total."<br>"."Aprobados: ".$aprobados."<br>"."No Aprobados: ".$no_aprobados."<br>"."Vacios: ".$vacios."<br>"."Tiempo total: ".$tiempototal." Segundos, o ".convertir_a_minutos($tiempototal)." Minutos<br><hr>";
        ...
        ?>
          ...
          <td><?php echo $aprobados ?></td>
          <td style="max-width: 200px;"><button class="btn btn-sucess" onclick="verocultar('detalles_est_<?php echo $row['id_matricula'] ?>')">Detalles</button>
          ...
          echo "<thead>";
          echo "<tr>";
              echo "<th>Secuencia</th>";
              echo "<th>Numero</th>";
              echo "<th>Aprobado</th>";
          echo "</tr>";
          echo "</thead>";
                echo "<tbody>";
           while($row_seguimiento_secuencia = $consulta_seguimiento_secuencia->fetch_assoc()){
                echo "<tr>";
                echo "<td>".consultar_nombre_secuencia($row_seguimiento_secuencia['id_secuencia'])."</td>";
                echo "<td>".$row_seguimiento_secuencia['numero']."</td>";
                echo "<td>".$row_seguimiento_secuencia['aprobado']."</td>";
                echo "</tr>";
            }
          echo "</tbody>";
        echo "</table>";
...
<div class="col-md-12">
    ...
    */
    ...
    ?>
    <div class="secuencia" title="<?php echo $reto['nombre_reto']; ?>">
        <div class="secuencia2" id="sec2<?php echo $id_reto ?>" >
        <span  title="Estado <?php echo $reto['estado']; ?>, Dificultad: <?php echo $reto['dificultad']; ?>" ondblclick="cambiar_estado_reto('<?php echo $id_reto ?>','<?php echo $reto['estado']; ?>');" class="badge estado_<?php echo $reto['estado']; ?>"><?php echo $reto['dificultad']; ?></span>
        <span style="background-color:#FFF;">
        </span>
        <!--cambiar_estado_reto(id,estado)-->
...
<input onclick="modificar_reto('<?php echo $id_reto ?>','<?php echo $reto['dificultad']; ?>','<?php echo $reto['estado']; ?>','<?php echo $reto['nombre_reto']; ?>')" style="float:right;margin-right:20px;width:18px;height:18px;" type="image" src="<?php echo $url_raiz ?>img/pencil.png" name="submit" id="submit" value="Modificar">
        <br>
        <?php 
...
<div id="slider">
  <input class="bar" style="width:240px" type="range" name="dificultad" id="dificultad" min="1" max="3"  required value="<?php echo $dif ?>" onchange="rangevalue.value=value" />
  <span class="highlight"></span>
  <output id="rangevalue"><?php echo $dif ?></output>
</div>
</p>
    <input type="button" onclick="guardar_reto();" id="btn_crear" value="Crear" value="Crear"></p>
    </div>
<?php
}
function modificar_reto($cod,$dif,$nombre=''){
require("conexion.php");
#$retos2[$cod] = array('id_reto'=>$cod,"nombre_reto"=>$cod,"dificultad"=>"1","estado"=>"");
$sql2 = "SELECT `elementos_juego`.`id_elementos_juego`, `elementos_reto`.`id_elementos_reto`, `elementos_reto`.`elemento_reto`, `elementos_reto`.`tipo`, `elementos_juego`.`tipo` as tipo_e_juego, `elementos_juego`.`nombre_elemento`, `elementos_juego`.`archivo` FROM `elementos_reto` left join `elementos_juego` on `elementos_reto`.`elemento_reto` = `elementos_juego`.`id_elementos_juego` WHERE `elementos_reto`.`reto` = '".$cod."';";
#echo $sql2;
$consulta2 = $mysqli->query($sql2);
?>
<div>
    ...
    <input class="" name="reto" id="reto" value="<?php if (isset($nombre)) 
        echo $nombre;
        else
        if (isset($proximo_id_reto)) echo $proximo_id_reto ?>">
    <input hidden name="secuencia" id="secuencia" value="<?php if (isset($_POST['secuencia'])) echo $_POST['secuencia'];?>" required>
    <br><input type="hidden" id="id_reto" name="id_reto" value="<?php if (isset($_POST['cod'])) echo $_POST['cod'];  ?>">
    </p>
        <div class="lienzo">
...
#$retos2[$cod]['elementos'][$row2['id_elementos_reto']]= array("elemento_reto"=>$row2['elemento_reto'],"tipo"=>$row2['tipo'],"tipo_e_juego"=>$row2['tipo_e_juego'],"nombre_elemento"=>$row2['nombre_elemento'],"archivo"=>$row2['archivo']);
        ...
        <label class="areaselementos" id="areaelemento<?php echo $cont ?>" for="radio<?php echo $cont ?>">
                ...
                ?>" type="hidden" name="<?php echo strtolower($row2['tipo_e_juego']) ?>[<?php echo $cont ?>]" 
                <?php if (strtolower($row2['tipo_e_juego'])!="figura"){ ?>
                id="<?php echo $row2['nombre_elemento'] ?>" value="<?php echo $row2['nombre_elemento'] ?>"
                <?php }else{ ?>
                id="<?php echo $row2['id_elementos_juego'] ?>" value="<?php echo $row2['id_elementos_juego'] ?>"
                <?php } ?>
                >
                <input required="" id="radio<?php echo $cont ?>" <?php
                if (strtolower($row2['tipo_e_juego'])!="figura") echo ' type="radio"';
                else echo 'type="hidden"';
                ?> class="correcta" name="correcta" <?php if ($row2['tipo']=="Clave") echo "checked";
                if (strtolower($row2['tipo_e_juego'])!="figura"){
                ?>
                value="<?php echo $row2['nombre_elemento'];?>"
                <?php }else{ ?>
                value="<?php echo $row2['nombre_elemento'];?>"
                <?php } ?>>
    	...
    	?><span><img src="<?php echo $url_raiz ?>img/figuras/<?php echo $row2['archivo'] ?>" width="120" style="margin-top: -50px;"></span><?php }else{ ?>
...
<div id="slider">
  ...
  <output id="rangevalue"><?php echo $dif ?></output>
    ...
    <input type="button" onclick="guardar_reto();" id="btn_crear" value="Guardar"></p>
    </div>
    <script>
...
function guardar_reto(){
    ...
    foreach ($_POST['figura'] as $id => $mifigura){
        $id_figura = $mifigura;//consulta_id_reto('Figura',$mifigura);
...
function consultar_nombre_secuencia($id){
	require ("conexion.php");
    $sql = "SELECT `nombre_secuencia` FROM `secuencia` WHERE `id_secuencia` = '".$id."' ";
     $consulta = $mysqli->query($sql);
    if ($row = $consulta->fetch_assoc()){
        return $row['nombre_secuencia'];
    }else{
        return $id;
    }
}
...
function cambiar_estado_reto($id, $estado="Borrador"){
    $estado = ($estado!="Borrador") ? "Borrador" : "Publicado";
    require ("conexion.php");
    @session_start();
    $sql ="UPDATE `secuencias`.`reto` SET `estado` = '$estado' WHERE `reto`.`id_reto` = '$id';";
    $consulta = $mysqli->query($sql);
    if ($mysqli->affected_rows>0){
    return $mysqli->insert_id;
    }else{
...
function guardar_elementos_juego($ajax=true){
require ("conexion.php");
@session_start();
  /*recibo los campos del formulario proveniente con el método POST*/ 
$sql = "INSERT INTO elementos_juego (`tipo`, `nombre_elemento` ";
if ($_POST['tipo']=="Figura") $sql .=", `archivo`";
$sql .=") VALUES ('".$_POST['tipo']."', '".$_POST['nombre_elemento']."'";
...
function nombre_grupo($id_grupo){
    require ("conexion.php");
    $sql_ac = "SELECT `nombre` FROM `grupo` WHERE `id_grupo` = '".$id_grupo."'";
...
if ($tabla){
?>
<h1>Tabla de Frecuencias para <?php echo $datos_a[0].": ".$nombre ?></h1>
<table border="1">
    <thead>
        <tr>
...
Graficar(titulo="Grafica de <?php echo $nombre ?>",cualitativa="Opción",cuantitativa="<?php echo $datos_a[1]?>",contenedor="div<?php echo $div ?>",tipo_grafica="<?php if ($total_datos<=5) echo "PieChart"; else echo "ColumnChart"; ?>",ancho="700px",alto="200px",datos<?php echo $div ?>);
});
</script>
...
function boton_modal_nuevo_icono(){
    ?>
    <button id="btn_nuevo_foro" type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal_nuevo">Nueva Figura</button>
    <?php
}
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Nueva Figura</h4>
      </div>
      <div class="modal-body">
        <form <?php echo $atributos ?> ENCTYPE="multipart/form-data">
              <div class="form-group">
                ...
                <input type="hidden" name="tipo" id="tipo" class="form-control" value="Figura">
                <input type="text" onchange="validar_elemento_juego();" onblur="this.onchange();" onkeyup="this.onchange();" name="nombre_elemento" id="nombre_elemento" class="form-control" placeholder="Nombre de Figura">
            </div>
            <div class="form-group" id="area_adjunto">
                <label for="archivo">Archivo</label>
                <input type="file" name="archivo" onchange="valida_adjunto_img(this);sugerir_nombre_adjunto();mostrarImagen(this);valida_elemento_juego();" id="archivo" class="form-control" placeholder="Titulo del Foro">
                <img width="80" id="img_archivo">
            </div>
            <div class="form-group">
                <p id="txt_valida_silaba"></p>
                <p id="txttiponombre_elemento"></p>
            </div>
            <div class="form-group">
