...
<?php if ($row['tipo']!="Vocal"){ ?>
<input title="Eliminar" type="image" src="<?php echo $url_raiz ?>img/eliminar.png" onClick="confirmeliminar('elementos_juego.php',{'del':'<?php echo $row['id_elementos_juego'];?>','tipo':'<?php echo $row['tipo'];?>','nombre_elemento':'<?php echo $row['nombre_elemento'];?>'},'<?php echo $row['tipo']." ".$row['nombre_elemento'];?>');" value="Eliminar">
 <?php } ?> 
</td>
<?php } ?>
</tr>
...
<label for="tipo">Tipo:<span title="Este campo es obligatorio" style="color:red;font-size: 30px;margin: 2px;/top: 12px;top: 13px;position: relative;">*</span></label>
<select title="" class="form-control" name="tipo" id="tipo"  required onchange="valida_elemento_juego(document.getElementById('nombre_elemento').value);setTimeout(function(){document.getElementById('nombre_elemento').onkeyup();},1);">
...
echo '<div class="form-group"><p><input title="" class="form-control" name="id_elementos_juego" type="hidden" id="id_elementos_juego" value="';if (isset($row['id_elementos_juego'])) echo $row['id_elementos_juego'];echo '"';echo '></p>';
echo "</div>";echo '<div class="form-group"><p>'; ?>
<label for="tipo">Tipo:<span title="Este campo es obligatorio" style="color:red;font-size: 30px;margin: 2px;/top: 12px;top: 13px;position: relative;">*</span></label><select title="" class="form-control" name="tipo" id="tipo"  required onchange="valida_elemento_juego(document.getElementById('nombre_elemento').value);setTimeout(function(){document.getElementById('nombre_elemento').onkeyup();},1000);">
<option value="">Seleccione una opci&oacute;n</option>
<option value="Silaba" <?php if (isset($row['tipo']) and $row['tipo'] =="Silaba") echo " selected "; ?> >Sílaba</option>
<option value="Figura" <?php if (isset($row['tipo']) and $row['tipo'] =="Figura") echo " selected "; ?> >Figura</option>
<option value="Palabra" <?php if (isset($row['tipo']) and $row['tipo'] =="Palabra") echo " selected "; ?> >Palabra</option>
...
<input title="" class="form-control" name="archivo" type="file" id="archivo" onchange="valida_adjunto_img(this);sugerir_nombre_adjunto();mostrarImagen(this);valida_elemento_juego();"></p>
</div>
...
if ($query_anterior = $mysqli->query($sql_anterior)) {
 if ($_FILES['archivo']['error']!=4 and $row_anterior = $query_anterior->fetch_assoc()){
...
<div class="form-group"><p>
 <label for="tipo">Tipo
 <select title="" class="form-control input_busqueda_avanzada" type="search" onchange="grabarcookie('busqueda_av_elementos_juegotipo',this.value);buscar();" onblur="this.onchange();">
<option value="">Seleccione una opci&oacute;n</option>
<option value="Silaba" <?php if (isset($_COOKIE['busqueda_av_elementos_juegotipo']) and $_COOKIE['busqueda_av_elementos_juegotipo'] =="Silaba") echo " selected "; ?> >Sílaba</option>
<option value="Figura" <?php if (isset($_COOKIE['busqueda_av_elementos_juegotipo']) and $_COOKIE['busqueda_av_elementos_juegotipo'] =="Figura") echo " selected "; ?> >Figura</option>
<option value="Palabra" <?php if (isset($_COOKIE['busqueda_av_elementos_juegotipo']) and $_COOKIE['busqueda_av_elementos_juegotipo'] =="Palabra") echo " selected "; ?> >Palabra</option>
<option value="Vocal" <?php if (isset($_COOKIE['busqueda_av_elementos_juegotipo']) and $_COOKIE['busqueda_av_elementos_juegotipo'] =="Vocal") echo " selected "; ?> >Vocal</option>
</select></label></p></div>
