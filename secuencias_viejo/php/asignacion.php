...
<?php 
echo '<div class="form-group"><p><input title="" class="form-control" name="id_asignacion" type="hidden" id="id_asignacion" value="';if (isset($row['id_asignacion'])) echo $row['id_asignacion'];echo '"';echo '></p>';
...
echo '</select></p>';
echo "</div>";echo '<div class="form-group"><p><label for="docente">Docente:<span title="Este campo es obligatorio" style="color:red;font-size: 30px;margin: 2px;/top: 12px;top: 13px;position: relative;">*</span></label>';
...
echo '</select></p>';
echo "</div>";
echo '<div class="form-group"><p><label for="anio">Año Lectivo:</label>';
...
echo "</div><span id=\"txtgrupodocenteanio\"></span>";
...
$cod = $_POST['cod'];
 ...
 @session_start(); 
$sql = "UPDATE asignacion SET id_asignacion='".$_POST['id_asignacion']."', grupo='".$_POST['grupo']."', docente='".$_POST['docente']."', anio='".$_SESSION['id_anio_lectivo']."'WHERE  `asignacion`.`id_asignacion` = '".$cod."';";
/* echo $sql;*/ 
 /*Se conecta a la BD y luego ejecuta la instrucción SQL*/ 
...
<div class="form-group"><p><label for="grupo">Grupo<input title="" class="form-control input_busqueda_avanzada" type="search" onchange="grabarcookie('busqueda_av_asignaciongrupo',this.value);buscar();" onblur="this.onchange();" value="
<?php if (isset($_COOKIE['busqueda_av_asignaciongrupo'])) echo $_COOKIE['busqueda_av_asignaciongrupo']; ?>
"></label></p></div>
<div class="form-group"><p><label for="docente">Docente<input title="" class="form-control input_busqueda_avanzada" type="search" onchange="grabarcookie('busqueda_av_asignaciondocente',this.value);buscar();" onblur="this.onchange();" value="
<?php if (isset($_COOKIE['busqueda_av_asignaciondocente'])) echo $_COOKIE['busqueda_av_asignaciondocente']; ?>
"></label></p></div>
<div class="form-group"><p><label for="anio">Año Lectivo<input title="" class="form-control input_busqueda_avanzada" type="search" onchange="grabarcookie('busqueda_av_asignacionanio',this.value);buscar();" onblur="this.onchange();" value="
