...
<img height="80" src="../img/Avatars/<?php echo $row['avatarimg_avatar']?>">
<?php } ?>
</td>
<?php } ?>
...
<td data-label="Ultimo Inicio"><?php echo  date("d \\d\\e ".$meses[date("n",strtotime($row['ultimo_inicio']))]."  \\d\\e\\l \\a\\ñ\\o Y ",strtotime($row['ultimo_inicio'])); ?></td>
<?php if ($reporte==""){ ?>
...
$sql = 'SELECT `id_usuarios`, `nuip`, `nombre`, `apellido1`, `apellido2`, `email`, `clave`, `f_nacimiento`, `tipo`, `avatar`, `puntos`, `estado`, `ultimo_inicio` FROM `usuarios` WHERE concat(`usuarios`.`id_usuarios`) ="'.$_POST['cod'].'" Limit 1'; 
$consulta = $mysqli->query($sql);
 /*echo $sql;*/ 
...
<?php 
echo '<div class="form-group"><p><input title="" class="form-control" name="id_usuarios" type="hidden" id="id_usuarios" value="';if (isset($row['id_usuarios'])) echo $row['id_usuarios'];echo '"';echo '></p>';
echo "</div>";echo '<div class="form-group"><p><label for="nuip">Identificación:<span title="Este campo es obligatorio" style="color:red;font-size: 30px;margin: 2px;/top: 12px;top: 13px;position: relative;">*</span></label><input title="Ejemplo: 1085234567" onchange="valida_existe_nuip(this.value)" onkeyup="valida_existe_nuip(this.value)" onKeyPress="return soloNumeros(event)" pattern="[0-9]{8,12}"  class="form-control" name="nuip" autocomplete="off" type="text" id="nuip" step="1" value="';if (isset($row['nuip'])) echo $row['nuip'];echo '"';echo ' required ><span id="txtnuip"></span></p>';
...
<span id="icono"></span>
<input onchange="document.getElementById('icon-img').src = this.value" type="hidden" id="icon" name="icon" autocomplete="off" value=""/>
...
<div class="container">
  ...
  <ul class="bs-glyphicons">
	...
	?>
	<li><span onclick="obtener_icono(this)" data-src="../img/Avatars/<?php echo $row_av['img_avatar'] ?>" data-id="<?php echo $row_av['id_avatar'] ?>" ><img height="80" src="../img/Avatars/<?php echo $row_av['img_avatar'] ?>"><?php echo $row_av['nombre_avatar']?></span></li>
	<?php } ?>
	<?php } ?>
    </ul> </div>
